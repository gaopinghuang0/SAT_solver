...
class GRASP_Solver(object):
  ...
  def is_unit_clause(self, c):
    ...
    for lit in c.lits:
      ...
      #   assign_value = imply_var_dict[var]
      var_result = lit.result(assign_value)
      if var_result == 1:
        return False
      elif var_result is None:
    ...
    if count == 1:
      ...
      # clause is failed due to imply value
      return True


  def get_imply(self, c):
  ...
  def propogate(self):
    ...
    for c in self.clauses:
      ...
      dbg(c.to_list(), self.is_unit_clause(c), self.assigns)
  ...
  def assign_dont_care_vars(self):
    print 'TODO: assign_dont_care_vars'
